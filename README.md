...
| **조회 성능** | 복합 인덱스 `(recipient, created_at)` — **풀스캔 18ms → 0.24ms** | [3.2](#32-조회-인덱스-설계--explain-기반-검증) |
...
**실측 (26,806행, 최악 조건인 전체 매치 수신자 기준)**

| | Before (인덱스 없음) | After |
|------|-----|-----|
| 실행 계획 | `Table scan` → `Filter` (+ filesort) | `Index range scan (reverse)` → `Limit` |
| 실제 읽은 행 | 26,803 | **20** |
| actual time | 18~32ms | **0.24ms (약 100배)** |
| 정렬 비용 | 전체 정렬 | 0 (인덱스가 이미 정렬) |

> **고민 흔적**
> - *"인덱스 넣었는데 효과가 없다"의 원인 두 가지*: 
>   ① 측정 쿼리가 실제 API 쿼리와 달랐다(조건 하나만, ORDER BY·LIMIT 없음). 그 형태에서는 옵티마이저가 다른 계획을 고른다.
> - *인덱스 사용 ≠ 개선*: 조건이 `recipient=` 하나뿐이고 그 수신자가 전체 행의 100%인 쿼리에서는
>   인덱스 경유가 **오히려 1.7배 느렸다**(18ms → 32ms). 보조 인덱스는 "인덱스 → 실제 행" 왕복이 발생하므로
>   어차피 전부 읽어야 하는 경우엔 순차 풀스캔이 싸다. **판정 기준은 type/key 가 아니라 actual time 과 실제 읽은 행 수다.**
> - *LIMIT*: 매치가 26,803건이어도 `ORDER BY ... LIMIT 20` 이면 인덱스를 역방향으로 20칸만 읽고 종료된다
>   (`Backward index scan`). 즉 이 인덱스의 이득은 선택도가 아니라 **정렬 제거 + 조기 종료**에서 나온다.
...
| permitted-calls-in-half-open | 3 | 1건은 우연에 속을 수 있고, 많으면 아직 아픈 상대에 부담 |
...
> - *AOP 의존성 누락 시 조용히 무시된다*: `@CircuitBreaker` 는 프록시 기반이라 `spring-boot-starter-aop` 가 없으면
>   어노테이션이 동작하지 않는다(`@Transactional` 과 같은 성질). actuator 도 함께 넣어 상태를 관측 가능하게 했다.
> - *fallback 시그니처 규칙*: 원본과 같은 파라미터 + 마지막에 `Throwable`, 반환 타입 동일. 어긋나면 런타임에 fallback 을 찾지 못한다.
>   또한 클래스 레벨이 아니라 **메서드 레벨**에 붙여야 대상이 명확하다.
> - *sendmock 설정을 바꿨는데 반영되지 않은 사건*: 실패율을 0.60으로 수정했으나 프로세스를 재시작하지 않아
>   옛 값(0.10)으로 계속 동작했다. 스프링은 부팅 시점에 설정을 읽으므로, 실측 전에는 **설정이 아니라 실제 응답으로**
>   조건을 검증해야 한다(프로브 20회로 실패율 측정).
...
- 인덱스만으로는 부족하다 — 발행 완료 행 누적 자체를 줄이는 정리 배치가 함께 필요하다(7장 한계 참고).
...
> - *남은 문제 — 발행 시도 횟수 미집계*: `continue` 로 전면 정지는 막았지만, 영구 실패 메시지는 여전히 매 주기 재시도되며
>   로그를 오염시킨다. `attemptCount` 와 `DEAD` 상태를 도입해 한도 초과 시 조회 대상에서 제외하는 것이
>   완전한 해법이다(알림의 DLT 격리와 같은 구조). 현재는 미구현으로 남겼다 — 7장 참고.

### 3.8 발송 이력 기록 — "몇 번, 왜 실패했는가"를 조회로 답한다
...
| 측정 | 결과 |
|------|------|
| 복합 인덱스 적용 (3.2) | 26,803행 스캔 18ms → **20행 0.24ms (약 100배)** |
...
- **서킷 OPEN 중 재시도 소모**: backoff 를 80초로 늘려 완화했지만, OPEN 구간과 겹치는 재시도는 여전히 즉시 실패로 소모된다.
  근본 해법은 "서킷 차단으로 인한 실패는 재시도 횟수를 소모하지 않는" 정책이나, DLT 에서 자동 복구 큐로 재투입하는 구조다.
- **outbox 발행 시도 횟수 미집계**: `continue` 로 전면 정지는 막았지만(3.7) 영구 실패 메시지가 매 주기 재시도되며 로그를 오염시킨다.
  `attemptCount` + `DEAD` 상태를 도입해 한도 초과 시 조회 대상에서 제외하면(알림의 DLT 격리와 같은 구조) 완전히 정리된다.
- **outbox 정리 배치 미구현**: 발행 완료 행이 누적되면 인덱스가 비대해지고 백업·디스크 부담이 커진다.
  `(published, id)` 인덱스로 조회 비용은 해결했으나 누적 자체를 줄이는 "published 이고 N일 경과" 삭제 배치가 필요하다.
  아카이빙과 달리 보존 가치가 없으므로 이관이 아닌 삭제가 맞다.
- **아카이브 조회 경로 없음**: 이관된 데이터를 조회할 API 가 없다. 필요해지면 `NotificationPersistencePort` 에
  아카이브 조회 메서드를 추가하면 되고, 코어는 "어디에 저장돼 있는지" 모른 채로 유지된다.
- **커서 페이징의 제약**: 임의 페이지 점프와 전체 건수 표시가 불가능하다. 관리자 화면이 필요하면 offset 경로를 쓴다.
- **`slow-call-duration-threshold` 재검토 여지**: 정상 응답이 수십 ms 인데 4초까지 관대하다.
  정상 응답 시간 분포(p99)를 근거로 2초 수준까지 낮추면 초기 징후를 더 빨리 잡을 수 있다.
- **월별 파티셔닝 보류**: 데이터가 수천만 건 규모가 되면 재검토. 단 멱등성 유니크 제약과의 충돌(3.4 고민 흔적)을 먼저 해결해야 한다.

**v2 에서 이어지는 항목**

- 발송-커밋 사이의 중복 창(외부 API 의 `requestId` dedup 에 의존), PROCESSING 고아 정리,
  인증 도입 시 `GET /notifications/me` 전환.
